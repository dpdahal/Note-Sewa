...
class BooksController {
    ...
    async destroy(req, res) {
        let bookId = req.params.id;
        let totalFindData = await BookImage.find({bookId: bookId}).countDocuments();
        if (totalFindData) {
            let findImage = await BookImage.find({bookId: bookId});
            console.log(findImage);
            let totalImageDelete = 0;
            findImage.map(async (image) => {
                let filePath = process.cwd() + "\\public\\uploads\\books\\" + image.image;
    ...
    async addBookImage(req, res) {
        ...
        if (req.files) {
            req.files.map(async (file) => {
                ...
                bookImages = await BookImage.create({bookId: bookId, image: file.filename});
            });
        }
        res.status(200).json({bookImages: bookImages, success: "Image not inserted"});
    }
    ...
    async addBookOrder(req, res) {
        ...
        let checkBookOrder = await BookOrders.find({bookId: bookId, userId: userId}).countDocuments();
        if (checkBookOrder.length > 0) {
            await BookOrders.findOneAndUpdate({bookId: bookId}, {$inc: {quantity: 1}});
    ...
    async searchBooks(req, res) {
        ...
        console.log(search);
        let booksData = await Books.find({title: {$regex: search, $options: "i"}});
        res.status(200).json({books: booksData});
    }
    ...
    async addBookRating(req, res) {
        ...
        let checkBookRating = await BookRatings.find({bookId: bookId, userId: userId}).countDocuments();
        if (checkBookRating > 0) {
            await BookRatings.findOneAndUpdate({bookId: bookId, userId: userId}, {$set: {rating: rating}});
    ...
    async addBookReview(req, res) {
        ...
        let checkBookReview = await BookRatings.find({bookId: bookId, userId: userId}).countDocuments();
        if (checkBookReview > 0) {
            await BookRatings.findOneAndUpdate({bookId: bookId, userId: userId}, {$set: {review: review}});
